...
- `currentCardIndex` (number): Tracks the user's linear progression (0 to 4) when navigating the deck.
...
- `appView` (string): Controls the routing between checking for saved decks, input prompt, and deck review.
These states flow unidirectionally downward via props to the isolated presentation components. Data remains strictly volatile and is destroyed the moment the component tree unmounts unless explicitly saved to `localStorage`.

## 3. API Contract & Payload Pipeline
The data transmission layer connects directly to the `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=<apiKey>` endpoint via a native browser `fetch` POST request originating from the client.
- **Headers:** `Content-Type: application/json`. Auth is handled via the query parameter.
- **System Prompt Schema:** The prompt explicitly forces the LLM to return exactly 5 items. The structural contract is enforced via Gemini's `responseSchema` configuration as a pure JSON array of objects, where each object contains an `id`, `question`, and `answer`. 
- **Error Handling:** Network timeouts (configured to 30 seconds via `AbortController`) and HTTP anomalies (e.g., 400 Bad Request for API_KEY_INVALID, 429 Rate Limit) are caught, parsed, and thrown as distinct UI-friendly error messages.

## 4. Component Topology & UI Architecture
The component matrix is explicitly modularized:
- **`app/page.tsx`:** The root orchestrator. Manages all volatile state, orchestrates `localStorage` interactions, and controls the conditional rendering between the input view, deck review view, and saved deck prompt.
- **`components/TextInputEngine.tsx`:** The input layer. Handles paste sanitization, character limit boundaries (max 10,000), and primary submission logic constraints.
...
- **`utils/apiService.ts`:** The pure network module. Isolates all Gemini `fetch` logic, JSON hydration, and error routing away from the React component tree.
...
- **Data Persistence:** The application utilizes `localStorage` to optionally save the generated deck. Refreshing the page (F5) will prompt the user to load their saved deck if one exists.
- **Token Handling:** The Gemini API key is managed securely via environment variables (`NEXT_PUBLIC_GEMINI_API_KEY`). It must never be logged to the console or transmitted to any unauthorized server.
