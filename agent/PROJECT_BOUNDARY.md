...
```
[Textarea Input] → [Validation (50–10,000 chars)] → [fetch() to OpenAI API] → [Parse Response] → [5 × { id, question, answer }] → [React useState]
```

- **Input:** Plain-text string from the textarea, validated client-side.
- **Processing:** Single `POST` request to OpenAI Chat Completions API with the text and a system prompt instructing 5 Q&A pair extraction. The API key is securely retrieved from `localStorage`.
...
| **API Utilities** | A single utility module exporting a `generateFlashcards(text, apiKey)` function that handles the `fetch` call to OpenAI, response parsing, and error handling. | Must not import React or render any UI. Returns a plain data structure (`Promise<Card[]>`). |
...
| C-5 | **Direct OpenAI API Access.** The browser fetches directly from the OpenAI API endpoint. No backend proxy, no API route forwarding, no middleware transformation. | The `generateFlashcards()` utility calls `fetch('https://api.openai.com/v1/chat/completions', ...)` directly with the user-provided API key in the `Authorization` header. |
| C-6 | **Fixed Card Count.** Every generation produces exactly 5 flashcards. No configurable count, no variable output. | The system prompt sent to OpenAI explicitly requests 5 Q&A pairs. The response parser validates exactly 5 items. |
| C-7 | **Token Persistence.** The user provides their OpenAI API key at runtime, which is then persisted safely client-side. | The key is stored securely in `localStorage` according to the `save_token.md` protocol. No `.env` files containing API keys shipped to the client. |
...
| A-2 | **Users have a valid OpenAI API key.** The primary generation flow requires the user to provide their own API key at runtime. | Without a valid key, the system falls back to mock generation, producing structurally valid but pedagogically simplistic cards. |
| A-3 | **The OpenAI API is available and responsive.** The generation pipeline assumes the external API returns results within 30 seconds. | Network failures or API outages trigger the 30-second timeout, displaying an error message with a "Retry" option. The mock fallback is available as an alternative. |
